...
public class CrimsonArrowEntity extends PersistentProjectileEntity {
    public CrimsonArrowEntity(EntityType<CrimsonArrowEntity> type, World world) {
        super(type, world);
    }

    public CrimsonArrowEntity(World world, LivingEntity owner) {
        super(ModEntityTypes.CRIMSON_ARROW, owner, world);
    }
    ...
    private void addParticles(int amount) {
        int c = ModEffects.RAGE.getColor();
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) {
        ...
        if (entityHitResult.getEntity() instanceof LivingEntity l && l.isMobOrPlayer()) {
            l.removeStatusEffect(ModEffects.CONFUSION);
            l.addStatusEffect(new StatusEffectInstance(ModEffects.RAGE, 10 * 20));
        }
    }

    @Override
    protected ItemStack asItemStack() {
        return new ItemStack(ModItems.CRIMSON_ARROW);
    }
}
